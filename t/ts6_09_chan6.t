...
sub client_input {
  ...
  if ( $cmd eq '366' ) {
    $poe_kernel->post( 'client', 'send_to_server', { command => 'MODE', params => [ '#floobar', '+S' ], colonify => 0 } );
    return;
  }
  if ( $cmd eq 'MODE' && $params->[0] eq '#floobar' ) {
    pass($cmd);
    $poe_kernel->delay('_launch_client2', 1) if !$heap->{client2};
    return;
  }
  return;
}
...
sub client3_input {
  ...
  if ( $cmd eq '489' ) {
    pass("IRC$cmd");
    is($params->[0], 'custard', 'I am custard' );
    is($params->[1], '#floobar', 'Channel is correct' );
    is($params->[2], 'Cannot join channel (+S) - SSL/TLS required', 'Cannot join channel (+S) - SSL/TLS required');
