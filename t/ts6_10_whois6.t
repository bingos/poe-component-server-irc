...
sub client_input {
  ...
  if ( $cmd eq '329' ) {
    ...
    $poe_kernel->post( 'groucho', 'send_to_server', { prefix => '7UPAAAAAA', command => 'WHOIS', params => [ '9T9', 'bobbins' ] } );
    return;
  }
  return;
}
...
sub harpo_input {
  ...
  if ( $cmd eq 'WHOIS' ) {
    ...
    is( $params->[0], '9T9', 'Target is harpo' );
    is( $params->[1], 'bobbins', 'They are asking about bobbins' );
