...
sub ircd_listener_add {
    ...
    $heap->{ircd}->add_pseudo(
        {
            cmd     => 'IDENTIFY',
            prepend => 'IDENTIFY',
            name    => 'NickServ',
            target  => 'NickServ!nickserv@fake.server.irc',
        }
    );
    $heap->{ircd}->add_pseudo(
        {
            cmd     => 'CHANSERV',
            name    => 'ChanServ',
            target  => 'ChanServ!chanserv@chan.server.irc',
        }
    );
...
sub groucho_connected {
  ...
  $kernel->post( $sender, 'send_to_server', { prefix => '4AK', command => 'UID', params => [ 'NickServ', '2', ($uidts - 50), '+iow', 'nickserv', 'fake.server.irc', '0', '4AKAAAAAA', '0', 'NickServ' ], colonify => 1 } );
...
sub client3_input {
  ...
  if ( $cmd eq 'MODE' && $prefix =~ m'^custard' && $params->[1] eq '+i' ) {
    pass($cmd);
    $poe_kernel->post($sender, 'send_to_server', { command => 'IDENTIFY' } );
    return;
  }
  if ( $cmd eq '412' ) {
    pass("IRC$cmd");
    $poe_kernel->post($sender, 'send_to_server', { command => 'IDENTIFY', params => [ 'SecretPass' ] } );
    return;
  }
  if ( $cmd eq '440' ) {
    pass("IRC$cmd");
    is($params->[1],'ChanServ','ChanServ');
    is($params->[2],'Services are currently unavailable.','Services are currently unavailable.');
    $poe_kernel->post($sender, 'send_to_server', { command => 'MEMOSERV', params => [ 'GET' ] } );
    return;
  }
  if ( $cmd eq '421' ) {
    pass("IRC$cmd");
    is($params->[1],'MEMOSERV','MEMOSERV');
    is($params->[2],'Unknown command','Unknown command');
...
sub groucho_input {
  ...
  if ( $cmd eq 'PRIVMSG' ) {
    pass($cmd);
    is($prefix,'1FUAAAAAC','Correct Prefix');
    is($params->[0],'4AKAAAAAA','Addressed to NickServ');
    is($params->[1],'IDENTIFY SecretPass','Message with prepend is good');
    $poe_kernel->post('client3', 'send_to_server', { command => 'CHANSERV', params => [ 'INVITE #floobar' ] } );
    return;
  }
  return;
}
