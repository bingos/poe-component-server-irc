...
sub client_input {
  ...
  if ( $cmd eq '329' ) {
    ...
    $poe_kernel->post( $sender, 'send_to_server', { command => 'WHOIS', params => [ 'groucho', 'groucho' ] } );
    return;
  }
  if ( $cmd eq 'PART' ) {
    pass($cmd);
  ...
  if ( $cmd eq '311' ) {
    pass("IRC$cmd");
    is( $prefix, 'groucho.server.irc', 'Our server: listen.server.irc' );
    is( $params->[0], 'bobbins', 'It is me, bobbins' );
  ...
  if ( $cmd eq '313' ) {
    ...
    is( $params->[0], 'bobbins', 'It is me, bobbins' );
    is( $params->[1], 'groucho', 'It is him, groucho' );
    is( $params->[2], 'is an IRC Operator', 'Counted amongst the blessed' );
    return;
  }
  if ( $cmd eq '318' ) {
    pass("IRC$cmd");
...
sub groucho_input {
  ...
  if ( $cmd eq 'WHOIS' ) {
    pass($cmd);
    # :1FUAAAAAA WHOIS 7UP :groucho
    is( $params->[0], '7UP', 'Addressed to me' );
    is( $prefix, '1FUAAAAAA', 'From: 1FUAAAAAA' );
    is( $params->[1], 'groucho', 'They are querying groucho' );
    $poe_kernel->post( $sender, 'send_to_server', { prefix => '7UP', command => '311', params => [ '1FUAAAAAA', 'groucho', 'groucho', 'groucho.marx', '*', 'Groucho Marx' ], colonify => 1 } );
    $poe_kernel->post( $sender, 'send_to_server', { prefix => '7UP', command => '319', params => [ '1FUAAAAAA', 'groucho', '@#marxbros' ], colonify => 1 } );
    $poe_kernel->post( $sender, 'send_to_server', { prefix => '7UP', command => '312', params => [ '1FUAAAAAA', 'groucho', 'groucho.server.irc', 'Open the door and come in!!!!!!' ], colonify => 1 } );
    $poe_kernel->post( $sender, 'send_to_server', { prefix => '7UP', command => '313', params => [ '1FUAAAAAA', 'groucho', 'is an IRC Operator' ], colonify => 1 } );
    $poe_kernel->post( $sender, 'send_to_server', { prefix => '7UP', command => '318', params => [ '1FUAAAAAA', 'groucho', 'End of /WHOIS list.' ], colonify => 1 } );
    return;
  }
  if ( $cmd eq 'PART' ) {
    pass($cmd);
