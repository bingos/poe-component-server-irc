...
sub client_input {
  ...
  if ( $cmd eq 'PRIVMSG' && $params->[0] =~ m!\$! ) {
    pass($cmd);
    is($prefix,'groucho!groucho@groucho.marx','groucho!groucho@groucho.marx');
    is($params->[0],'$listen.server.irc','$listen.server.irc');
    is($params->[1],'Howdy there','Howdy there');
    return;
  }
  if ( $cmd eq 'NOTICE' && $params->[0] =~ m!\$! ) {
    pass($cmd);
    is($prefix,'groucho!groucho@groucho.marx','groucho!groucho@groucho.marx');
    is($params->[0],'$#listen.server.irc','$#listen.server.irc');
    is($params->[1],'Listening to me','Listening to me');
    return;
  }
  if ( $cmd eq 'PRIVMSG' && $params->[0] =~ m!^#! ) {
    pass($cmd);
    is($prefix,'groucho!groucho@groucho.marx','groucho!groucho@groucho.marx');
    $poe_kernel->post( $sender, 'send_to_server', { command => 'PRIVMSG', params => [ '#floobar', 'Elephants are all pink!!!!' ] } );
    return;
  }
  return;
}
...
sub client3_input {
  ...
  if ( $cmd eq '366' ) {
    # Start testing here
    $poe_kernel->post('groucho', 'send_to_server', { prefix => '7UPAAAAAA', command => 'PRIVMSG', params => [ '$listen.server.irc', 'Howdy there' ] } );
    $poe_kernel->post('groucho', 'send_to_server', { prefix => '7UPAAAAAA', command => 'NOTICE', params => [ '$#listen.server.irc', 'Listening to me' ] } );
    $poe_kernel->post('groucho', 'send_to_server', { prefix => '7UPAAAAAA', command => 'JOIN', params => [ $heap->{chants}, '#floobar', '+' ], colonify => 0 } );
    return;
  }
  if ( $cmd eq 'PRIVMSG' && $params->[0] =~ m!\$! ) {
    pass($cmd);
...
sub groucho_input {
  ...
  if ( $cmd eq 'SJOIN' ) {
    pass($cmd);
    $heap->{chants} = $params->[0];
    return;
  }
  if ( $cmd eq 'PRIVMSG' ) {
    pass($cmd);
    is($prefix,'1FUAAAAAA','Correct Prefix');
    is($params->[0],'#floobar','Target is channel');
    is($params->[1],'Elephants are all pink!!!!','Elephants are all pink!!!!');
...
sub harpo_input {
  ...
  my $cmd    = $in->{command};
  my $params = $in->{params};
  if ( $cmd eq 'JOIN' && $prefix eq '7UPAAAAAA' ) {
    pass($cmd);
    $poe_kernel->post('groucho', 'send_to_server', { prefix => '7UPAAAAAA', command => 'PRIVMSG', params => [ '#floobar', 'Now I am here, suckers' ] } );
    return;
  }
  return;
}
