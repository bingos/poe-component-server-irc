...
sub client_input {
  ...
  if ( $cmd eq 'NOTICE' ) {
    ...
    is($params->[1],'*** Notice -- TRACE requested by bobbins (~bobbins@listen.server.irc) [listen.server.irc]',
      '*** Notice -- TRACE requested by bobbins (~bobbins@listen.server.irc) [listen.server.irc]');
    return;
  }
  if ( $cmd eq '205' ) {
    pass("IRC$cmd");
