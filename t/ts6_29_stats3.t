...
sub client_input {
  ...
  if ( $cmd eq 'MODE' && $prefix =~ m'^bobbins' && $params->[1] eq '+i' ) {
    $poe_kernel->post($sender, 'send_to_server', { command => 'STATS', params => [ 'u', 'fake*' ], colonify => 0 } );
    return;
  }
  if ( $cmd eq 'ERROR' ) {
    pass($cmd);
...
sub groucho_input {
  ...
  # :1FUAAAAAA STATS u :4AK
  if ( $cmd eq 'STATS' ) {
    pass($cmd);
    is($params->[0],'u','Uptime request');
    is($params->[1],'4AK','Target is fake');
    my $uptime = time - ( 86400 * 4 );
    ...
    $remain   %= 60;
    $poe_kernel->post($sender, 'send_to_server', {
      prefix  => '4AK',
      command => '242',
      params  => [
                $prefix,
