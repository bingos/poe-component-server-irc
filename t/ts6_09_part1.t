...
sub client3_input {
  ...
  if ( $cmd eq '366' ) {
    $poe_kernel->post('client2', 'send_to_server', { command => 'PART', params => [ '#floobar', 'Filthy ape' ], colonify => 0 } );
    return;
  }
  if ( $cmd eq 'PART' ) {
    pass($cmd);
    is($prefix,'rubbarb!~rubbarb@listen.server.irc','rubbarb!~rubbarb@listen.server.irc');
    is($params->[0],'#floobar','Channel is good');
