...
sub client_input {
  ...
  if ( $cmd eq 'MODE' && $prefix =~ m'^bobbins' && $params->[1] eq '+i' ) {
    pass($cmd);
    $heap->{ircd}->daemon_server_join('bobbins','#floobar');
...
sub client2_input {
  ...
  if ( $cmd eq 'MODE' && $prefix =~ m'^rubbarb' && $params->[1] eq '+i' ) {
    pass($cmd);
    $heap->{ircd}->daemon_server_join('rubbarb','#floobar');
    return;
  }
  if ( $cmd eq '366' ) {
  ...
  if ( $cmd eq 'PRIVMSG' && $params->[1] =~ m!ACTION! ) {
    ...
    $heap->{ircd}->daemon_server_mode('#floobar', '+M');
    return;
  }
...
sub client3_input {
  ...
  if ( $cmd eq 'MODE' && $prefix =~ m'^custard' && $params->[1] eq '+i' ) {
    pass($cmd);
    $heap->{ircd}->daemon_server_join('custard','#floobar');
  }
  if ( $cmd eq '366' ) {
    $heap->{ircd}->daemon_server_mode('#floobar', '+C');
    return;
  }
  ...
  if ( $cmd eq 'MODE' && $params->[1] eq '-N+u' ) {
    $heap->{ircd}->daemon_server_mode('#floobar', '+b', 'bads!bads@bads.net');
    return;
  }
  ...
  if ( $cmd eq '477' ) {
    ...
    if ( !$heap->{477} ) {
      $heap->{ircd}->daemon_server_mode('#floobar', '-M+T');
      $heap->{477}++;
      return;
    }
  ...
  if ( $cmd eq '404' ) {
    ...
    $heap->{ircd}->daemon_server_mode('#floobar', '-T+N');
    return;
  }
  if ( $cmd eq '447' ) {
    pass("IRC$cmd");
    ...
    $heap->{ircd}->daemon_server_mode('#floobar', '-N+u');
    return;
  }
  if ( $cmd eq '368' ) {
    pass("IRC$cmd");
    ...
    $heap->{ircd}->daemon_server_mode('#floobar', '+R');
    return;
  }
  if ( $cmd eq '367' ) {
