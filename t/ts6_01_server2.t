...
sub groucho_input {
  ...
  if ( $cmd eq 'SQUIT' ) {
    ...
    is( $params->[1], 'Link cancelled, server ID already exists', 'Link cancelled, server ID already exists' );
...
sub ircd_daemon_eob {
  ...
  if ( $heap->{eob} >= 3 ) {
    $kernel->state('_bogus_server', \&_bogus_server);
    $kernel->delay('_bogus_server',3);
  }
  return;
}

sub _bogus_server {
    $poe_kernel->post( 'harpo', 'send_to_server', { prefix => '9T9', command => 'SID', params => [ 'fake.server.irc', 2, '4AK', 'This is a fake server' ] } );
    return;
}
