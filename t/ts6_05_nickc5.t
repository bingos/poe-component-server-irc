...
sub harpo_connected {
  ...
  $kernel->post( $sender, 'send_to_server', { prefix => '9T9', command => 'UID', params => [ 'bobbins', '1', $uidts, '+i', 'pobbins', 'different.host.irc', '192.168.1.40', '9T9AAAAAB', '0', 'bobbins along' ], colonify => 1 } );
...
sub groucho_input {
  ...
  my $cmd    = $in->{command};
  my $params = $in->{params};
  if ( $cmd eq 'UID' && $params->[7] eq '9T9AAAAAB' ) {
    fail('9T9AAAAAB wrongly won collision');
    return;
  }
  if ( $cmd eq 'QUIT' ) {
    pass($cmd);
...
sub harpo_input {
  ...
  if ( $cmd eq 'KILL' ) {
    pass($cmd);
    is( $prefix, '1FU', 'Prefix is 1FU' );
    is( $params->[0], '9T9AAAAAB', 'Our version of bobbins' );
    is( $params->[1], 'Nick Collision', 'Nick Collision' );
