...
sub ircd_listener_add {
    ...
    $heap->{port} = $port;
    $heap->{ircd}->add_auth(
        mask     => '*teapot@127.0.0.1',
        password => '$2a$06$M1EiLiQH6Ro8DAwmwVGd8uwgFomXGUAokSdsSN/L2kXXL8Mfe35wq',
        no_tilde => 1,
        spoof    => 'funny.host.name',
    );
...
sub client_connected2 {
  my ($kernel,$heap,$sender) = @_[KERNEL,HEAP,SENDER];
  pass($_[STATE]);
  $kernel->post( $sender, 'send_to_server', { command => 'NICK', params => [ 'bobbins' ], colonify => 0 } );
  $kernel->post( $sender, 'send_to_server', { command => 'USER', params => [ 'teapot', '*', '*', 'bobbins along' ], colonify => 1 } );
  return;
}

sub client_connected3 {
  my ($kernel,$heap,$sender) = @_[KERNEL,HEAP,SENDER];
  pass($_[STATE]);
  $kernel->post( $sender, 'send_to_server', { command => 'PASS', params => [ 'fubar123' ], colonify => 0 } );
  $kernel->post( $sender, 'send_to_server', { command => 'NICK', params => [ 'bobbins' ], colonify => 0 } );
  $kernel->post( $sender, 'send_to_server', { command => 'USER', params => [ 'teapot', '*', '*', 'bobbins along' ], colonify => 1 } );
  return;
}

sub client_connected4 {
  my ($kernel,$heap,$sender) = @_[KERNEL,HEAP,SENDER];
  pass($_[STATE]);
  $kernel->post( $sender, 'send_to_server', { command => 'PASS', params => [ 'letmein' ], colonify => 0 } );
  $kernel->post( $sender, 'send_to_server', { command => 'NICK', params => [ 'bobbins' ], colonify => 0 } );
  $kernel->post( $sender, 'send_to_server', { command => 'USER', params => [ 'teapot', '*', '*', 'bobbins along' ], colonify => 1 } );
  return;
}
...
sub client_disconnected {
  ...
  pass($state);
  $poe_kernel->state('client_connected','main','client_connected2');
  $poe_kernel->state('client_disconnected','main','client_disconnected2');
  $poe_kernel->post( $sender, 'connect' );
  return;
}
...
sub client_disconnected2 {
  my ($heap,$state,$sender) = @_[HEAP,STATE,SENDER];
  pass($state);
  $poe_kernel->state('client_connected','main','client_connected3');
  $poe_kernel->state('client_disconnected','main','client_disconnected3');
  $poe_kernel->post( $sender, 'connect' );
  return;
}

sub client_disconnected3 {
  my ($heap,$state,$sender) = @_[HEAP,STATE,SENDER];
  pass($state);
  $poe_kernel->state('client_connected','main','client_connected4');
  $poe_kernel->state('client_input','main','client_input4');
  $poe_kernel->state('client_disconnected','main','client_disconnected4');
  $poe_kernel->post( $sender, 'connect' );
  return;
}

sub client_disconnected4 {
  my ($heap,$state,$sender) = @_[HEAP,STATE,SENDER];
  pass($state);
