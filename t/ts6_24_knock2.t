...
sub groucho_connected {
  ...
  $kernel->post( $sender, 'send_to_server', { prefix => '7UP', command => 'SJOIN', params => [ $chants, '#marxbros', '+lnt', '40', '@7UPAAAAAA' ], colonify => 1 } );
...
sub harpo_connected {
  ...
  $kernel->post( $sender, 'send_to_server', { prefix => '9T9', command => 'SJOIN', params => [ $chants, '#marxbros', '+lnt', '40', '9T9AAAAAA' ], colonify => 1 } );
...
sub client_input {
  ...
  if ( $cmd eq '713' ) {
    pass("IRC_$cmd");
    is($params->[1],'#marxbros','Channel is good');
    is($params->[2],'Channel is open.','Channel is open.');
