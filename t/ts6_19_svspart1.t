...
sub client_input {
  ...
  if ( $cmd eq 'MODE' && $prefix =~ m'^bobbins' && $params->[1] eq '+i' ) {
    pass($cmd);
    $poe_kernel->post( $sender, 'send_to_server', { command => 'JOIN', params => [ '#smurfs' ], colonify => 0 } );
    return;
  }
  if ( $cmd eq 'PART' ) {
    pass("$cmd");
  ...
  if ( $cmd eq '366' ) {
    ...
    $poe_kernel->post('groucho', 'send_to_server', { prefix => '4AK', command => 'SVSPART', params => [ '1FUAAAAAA', '#smurfs', 'Buh-bye' ] } );
    return;
  }
  return;
}
...
sub groucho_input {
  ...
  if ( $cmd eq 'PART' ) {
    ...
    is( $params->[0], q{#smurfs}, 'Here be smurfs' );
    return;
  }
  if ( $cmd eq 'QUIT' ) {
    pass($cmd);
