...
my $pocosi = POE::Component::Server::IRC->spawn(
    ...
    antiflood    => 0,
    plugin_debug => 1,
    config => { servername => 'listen.server.irc', sid => '1FU', anti_spam_exit_message_time => 0, hidden => 1 },
);
...
sub groucho_connected {
  ...
  $uidts = time() - 20;
  $kernel->post( $sender, 'send_to_server', { prefix => '7UP', command => 'SID', params => [ 'fake.server.irc', 2, '4AK', '(H) This is a fake server' ] } );
...
sub groucho_input {
  ...
  if ( $cmd eq 'SERVER' ) {
    pass($cmd);
    like( $params->[2], qr/^\(H\) /, 'Hidden server' );
    return;
  }
  return;
}
...
sub harpo_input {
  ...
  if ( $cmd eq 'SID' ) {
    pass($cmd);
    return if $params->[0] ne 'fake.server.irc';
    like( $params->[3], qr/^\(H\) /, 'Hidden server' );
    my $state = $heap->{ircd}{state};
    is( $state->{sids}{'4AK'}{hidden}, 1, 'Defined as hidden server in state' );
    unlike( $state->{sids}{'4AK'}{desc}, qr/^\(H\) /, 'The hidden hack should have been stripped' );
    return;
  }
  if ( $cmd eq 'SERVER' ) {
    pass($cmd);
