...
sub client4_connected {
  my ($kernel,$heap,$sender) = @_[KERNEL,HEAP,SENDER];
  pass($_[STATE]);
  $poe_kernel->post( 'client', 'send_to_server', { command => 'ETRACE' } );
  return;
}
...
sub client_input {
  ...
  if ( $cmd eq 'NOTICE' ) {
    ...
    is($params->[1],'*** Notice -- ETRACE requested by bobbins (~bobbins@listen.server.irc) [listen.server.irc]',
      '*** Notice -- ETRACE requested by bobbins (~bobbins@listen.server.irc) [listen.server.irc]');
    return;
  }
  if ( $cmd eq '709' ) {
    pass("IRC$cmd");
  ...
  if ( $cmd eq '759' ) {
    pass("IRC$cmd");
    is($params->[0],'bobbins','Got the NICK right');
