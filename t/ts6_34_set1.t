...
my @notices = (
':listen.server.irc NOTICE bobbins :Available QUOTE SET commands:',
':listen.server.irc NOTICE bobbins :FLOODCOUNT FLOODTIME IDENTTIMEOUT JFLOODCOUNT',
':listen.server.irc NOTICE bobbins :JFLOODTIME MAX SPAMNUM SPAMTIME',
':listen.server.irc NOTICE bobbins :JFLOODTIME is currently 6',
':listen.server.irc NOTICE * :*** Notice -- bobbins!~bobbins@listen.server.irc{moo} has changed JFLOODTIME to 8',
':listen.server.irc NOTICE bobbins :MAXCLIENTS is currently 512',
':listen.server.irc NOTICE bobbins :You cannot set MAXCLIENTS to < 32, restoring to 512',
);
...
sub client_input {
  ...
  if ( $cmd eq 'MODE' && $params->[0] !~ m!^#! ) {
    pass($cmd);
    is($params->[1],'+acosy','Correct OPER umodes set');
    $poe_kernel->post( $sender, 'send_to_server', { command => 'SET' } );
    $poe_kernel->post( $sender, 'send_to_server', { command => 'SET', params => [ 'JFLOODTIME' ], colonify => 0 } );
    $poe_kernel->post( $sender, 'send_to_server', { command => 'SET', params => [ 'JFLOODTIME', 8 ], colonify => 0 } );
    $poe_kernel->post( $sender, 'send_to_server', { command => 'SET', params => [ 'MAX' ], colonify => 0 } );
    $poe_kernel->post( $sender, 'send_to_server', { command => 'SET', params => [ 'MAX', 30 ], colonify => 0 } );
    return;
  }
  if ( $cmd eq 'NOTICE' && scalar @notices ) {
    pass($cmd);
    my $notice = shift @notices;
    is($in->{raw_line},$notice,$notice);
    return if scalar @notices;
