...
sub client_input {
  ...
  if ( $cmd eq 'MODE' && $prefix =~ m'^bobbins' && $params->[1] eq '+i' ) {
    pass($cmd);
    $poe_kernel->post( $sender, 'send_to_server', { command => 'ETRACE' } );
  }
  if ( $cmd eq '481' ) {
    pass("IRC$cmd");
    is($params->[0],'bobbins','Got the NICK right');
    is($params->[1],q{Permission Denied- You're not an IRC operator},q{Permission Denied- You're not an IRC operator});
