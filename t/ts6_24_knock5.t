...
sub client_input {
  ...
  if ( $cmd eq 'NOTICE' ) {
    pass($cmd);
    is($prefix,'listen.server.irc','NOTICE from server');
    is($params->[0],'#floobar','Target is the channel');
    is($params->[1],'KNOCK: #floobar (groucho [groucho@groucho.marx] has asked for an invite)','KNOCK: #floobar (groucho [groucho@groucho.marx] has asked for an invite)');
...
sub client3_input {
  ...
  if ( $cmd eq '366' ) {
    # Start testing here
    $poe_kernel->post( 'client', 'send_to_server', { command => 'MODE', params => [ '#floobar', '+i' ], colonify => 0 } );
    return;
  }
  if ( $cmd eq 'NOTICE' ) {
    fail('Should not get a NOTICE to a non-op/hop');
    return;
  }
  return;
}
...
sub groucho_input {
  ...
  if ( $cmd eq 'TMODE' && $params->[2] eq '+i' ) {
    pass($cmd);
    $poe_kernel->post( $sender, 'send_to_server', { prefix => '7UPAAAAAA', command => 'KNOCK', params => [ '#floobar' ], colonify => 0 } );
    return;
  }
  return;
}
